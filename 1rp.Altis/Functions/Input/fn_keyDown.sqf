...
_disabledKeys append (actionKeys "tacticalView"); // Отключить тактический вид...
    ...
    switch (_code) do {
        ...
        case G: {
            if (_shift) then {
                if !((currentWeapon player) in getArray(missionConfigFile >> "CfgSettings" >> "doesntThreaten")) then {
                    [cursorObject] call ULP_fnc_knockOut; // Скрипт выполняет проверки...
                };
                _handled = true;
            };
        };
        ...
        case F: {
            if (!_alt && { !(isNull (objectParent player)) }) then {
                ...
                // Ничего этого не может произойти без них
                ...
                if (isClass _vehSiren && { (driver _veh) isEqualTo player }) then {
                    ...
                    if (isNull (_veh getVariable ["siren", objNull])) then {
                        if (_shift && { isNil "_timeout" || { time > _timeout } }) then {
                            titleText["Сирены включены", "PLAIN"];
                        ...
                        if (_shift) then {
                            ...
                            titleText["Сирены выключены", "PLAIN"];
            ...
            if (_shift && { !_alt } && { !_ctrlKey } && { !(isNull (objectParent player)) }) then {
                ...
                if (isClass _vehLights && { (driver _veh) isEqualTo player }) then {
                    if (isNull (_veh getVariable ["lightLoop", scriptNull])) then {
                        ...
                        titleText["Фары выключены", "PLAIN"];
