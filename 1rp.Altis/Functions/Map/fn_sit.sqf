...
switch _mode do {
	case "init": {
    	_params params [["_chair", cursorObject, [objNull]], ["_unit", player, [objNull]]];
   		_chair addAction ["Сесть", {["sitDown", _this] call ULP_fnc_sit}, [], 0, true, true, "", "!(_target getVariable [""occupied"", false]) && { isNil ""isSitting"" }", 2];
  	};
  	...
  	case "sitDown": {
    	...
    	_player addAction ["Встать", {(_this # 3) params ["_chair", "_player"]; ["standUp", [_chair, _player, _this # 2]] call ULP_fnc_sit}, [_chair, _player]];
 	};
