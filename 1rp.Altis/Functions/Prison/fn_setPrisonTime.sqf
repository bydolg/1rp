...
if !([player] call ULP_fnc_canImprison || { [_target] call ULP_fnc_canImprisoned }) exitWith {
	hint "Вы не можете отправить кого-либо в тюрьму отсюда...";
	["Вы не можете отправить кого-либо в тюрьму отсюда!"] call ULP_fnc_hint;
};
...
[
	(findDisplay getNumber(configFile >> "RscDisplayMission" >> "idd")), [1, _max], [_target, _prison, _max],
	{
		...
		if (_duraction > _max) exitWith {
			[format ["Вы можете приговорить кого-либо максимум на <t color='#B92DE0'>%1</t>", [_max, "MM:SS"] call BIS_fnc_secondsToString]] call ULP_fnc_hint;
		};
		...
		[format["Вы приговорили <t color='#B92DE0'>%1</t> на <t color='#B92DE0'>%2</t>", name _target, [_duraction, "MM:SS"] call BIS_fnc_secondsToString]] call ULP_fnc_hint;
