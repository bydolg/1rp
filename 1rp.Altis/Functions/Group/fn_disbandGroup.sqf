...
if !([] call ULP_fnc_isGroupOwner) exitWith {
    ["Вы должны быть владельцем группы, чтобы распустить ее!"] call ULP_fnc_hint;
    false
};
...
ULP_DisbandEvent = ["GroupDisbanded", {
    ...
    ULP_HouseSellEvent = ["HouseSold", {
        ...
        if (_success) then { [_money, true, "Дом продан"] call ULP_fnc_addMoney; };

        ULP_HousesToSell = ULP_HousesToSell - 1;
        if (ULP_HousesToSell < 1) then {
            ["Все зависящие от группы дома проданы!"] call ULP_fnc_hint;
...
[
    ...
    format ["Вы уверены, что хотите <t color='#B92DE0'>распустить %1</t>...", [_group] call ULP_fnc_getGroupName], [_group],
    {   
        _this params [ "_group" ];
