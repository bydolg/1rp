...
_display setVariable ["community_goals_received", (["CommunityGoalsReceived", {
		...
		// Если наш ctrlGroup не отображается, значит, мы больше не находимся на этой странице, и этот обработчик событий не должен находиться здесь
...

		{
			...
			if (isClass _goalCfg && { [player, getArray (_goalCfg >> "factions")] call ULP_fnc_isFaction }) then {
				...
				_descCtrl ctrlSetStructuredText parseText format["Текущий Выплаты: <t color='#B92DE0'>%1%2</t><br/>Ваша Выплаты: <t color='#B92DE0'>%1%3</t>", 
...

				{
					...
					_line ctrlSetTooltip format["Уровень %1", _forEachIndex + 1];
					_line ctrlCommit 0;
				...
				_proTxtCtrl ctrlSetTooltip format ["Ваш Вклад (%1%2)", (_myProgress / (_progress max 1)) * 100, "%"];
