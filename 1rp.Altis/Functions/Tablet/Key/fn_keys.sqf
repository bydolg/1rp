...

		{
			if !(isNull _x) then {
				...
				if !(_cfg isEqualTo []) then {
					...
					_houseList lbSetTooltip [_item, format ["Расстояние: %1м", [_distance] call ULP_fnc_numberText]];
				};
			};
		} forEach ULP_Houses;
...

		{
			if !(isNull _x) then {
				...
				if !(_vehInfo isEqualTo []) then {
					...
					_keyList lbSetTooltip [_item, format["Расстояние: %1м", [[_distance] call ULP_fnc_numberText, ">100"] select (_distance >= 100)]];
