...

{
	...
	if (_state isEqualTo 2) then {
		...
		_coverCtrl ctrlSetTooltip format ["Требуется %1 %2", _requirement select 0, _requirement select 1];
		...
		if (_maxLevel > 1) then {
			...
			_ctrlLevel ctrlSetTooltip format["%1 XP до следующего уровня", _requiredXp - _xp];
	...
	if (_state isEqualTo 0) then {
		...
		_button ctrlSetTooltip (["Деактивировать перк", format ["Стоимость удаления: %1%2", "$", [_cost] call ULP_fnc_numberText]] joinString "\n");
	} else {
