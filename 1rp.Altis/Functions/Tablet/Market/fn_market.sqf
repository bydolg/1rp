...
(_display displayCtrl 23001) ctrlSetStructuredText parseText format["<t align='left'>Рынок</t><t align='right'>Налог: %1%2</t>", (["Налог", 5] call ULP_fnc_getLegislation), "%"];
	...
	if !([getNumber (_x >> "Settings" >> "isScripted")] call ULP_fnc_bool) then {
		...
		if !(_itemIcon isEqualTo "") then {
			...
			if ((configName _x) isEqualTo (missionNamespace getVariable ["ULP_SRV_Setting_BuffedItem", ""])) then {
				...
				_list lnbSetTooltip [[_row, 0], "Этот предмет имеет повышенную стоимость продажи..."];
			};
		};
