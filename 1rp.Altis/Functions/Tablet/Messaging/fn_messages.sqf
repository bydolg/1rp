...
** Описание: 
** Примечание: Возможно, стоило создать отдельные страницы для этих двух функций, не осознал, сколько всего будет переключено, может быть, сделать это позже? TODO
*/
...
// Заполнение мест для редактирования...
...
switch (lbCurSel _toolbox) do {
	case 1: {
		_sendButton ctrlSetStructuredText parseText "<t align = 'center'>Ответить</t>";
...
// Включение созданных кнопок переключения...
