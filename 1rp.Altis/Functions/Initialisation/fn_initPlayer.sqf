...
if (_addActions) then {
	...
	ULP_PlayerActions pushBackUnique (player addAction ["<img image='\a3\ui_f\data\IGUI\Cfg\simpleTasks\types\heal_ca.paa'/> Лечить себя", { player call ULP_fnc_healAction }, nil, 5, true, true, "", "isNull (objectParent player) && { !([] call ULP_UI_fnc_isProgress) } && { [player] call ULP_fnc_canHeal }"]);
	ULP_PlayerActions pushBackUnique (player addAction ["<img image='\a3\ui_f\data\IGUI\Cfg\simpleTasks\types\heal_ca.paa'/> Лечить", { cursorObject call ULP_fnc_healAction }, nil, 5, true, true, "", "isNull (objectParent cursorObject) && { !([] call ULP_UI_fnc_isProgress) } && { [cursorObject] call ULP_fnc_canHeal }"]);
