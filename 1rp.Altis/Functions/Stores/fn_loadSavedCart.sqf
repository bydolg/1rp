...
if (_selectableCarts isEqualTo []) exitWith {
	["У вас нет сохраненных корзин"] call ULP_fnc_hint;
};
...
[(findDisplay getNumber(configFile >> "RscDisplayMission" >> "idd")), _selectableCarts, 
	...
	if (_index isEqualTo -1) exitWith { ["Вы не выбрали корзину для загрузки"] call ULP_fnc_hint; };
	...
	[format ["Загружена корзина '%1'", _selectedCartName]] call ULP_fnc_hint;
