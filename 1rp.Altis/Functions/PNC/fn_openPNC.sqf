...
if (time < (player getVariable ["pnc_cd", 0])) exitWith {
	["Вы временно заблокированы в PNC..."] call ULP_fnc_hint;
};

if ([player] call ULP_fnc_isRestrained) exitWith {
	["Вы не можете получить доступ, находясь в состоянии заключения!"] call ULP_fnc_hint;
};

if (createDialog "DialogPNC") then {
	private _display = findDisplay 4700;

	(_display displayCtrl 4701) ctrlSetStructuredText parseText format ["Полицейский Национальный Компьютер<t align='right'>Уровень угрозы: %1</t>",
	...
	[_display, _login, _main, _userInput, _passInput] spawn {
		...
		if !(_loggedIn) exitWith {
			["Вам не удалось взломать PNC..."] call ULP_fnc_hint;
