...
[(findDisplay getNumber(configFile >> "RscDisplayMission" >> "idd")), ["Civilian"], [_display], {
	...
	if (isNull _unit) exitWith {
		["Вы не выбрали никого для передачи!"] call ULP_fnc_hint;
	};
	...
	[0.01, [_pnc, _unit], {
		[(findDisplay getNumber(configFile >> "RscDisplayMission" >> "idd")), _this, {
			...
			if (_length < 0 || { _length > _max }) exitWith {
				[format ["Примечание не может быть длиннее %1 символов...", [_max] call ULP_fnc_numberText]] call ULP_fnc_hint;
			};
			...
			[format ["Вы выдали или отредактировали ордер для <t color='#B92DE0'>%1</t>...", name _unit]] call ULP_fnc_hint;
