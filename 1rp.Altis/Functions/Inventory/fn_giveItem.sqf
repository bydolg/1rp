...
if (([_item] call ULP_fnc_hasItem) < _count) exitWith {
        [format ["У вас нет %1 %2 для передачи %3", [_count] call ULP_fnc_numberText, getText (_itemCfg >> "displayName"), [_unit, true] call ULP_fnc_getName]] call ULP_fnc_hint;
	false
};

[_item, _count, true] call ULP_fnc_handleItem;
[format ["Вы передали %1 %2 %3", [_unit, true] call ULP_fnc_getName, [_count] call ULP_fnc_numberText, getText (_itemCfg >> "displayName")]] call ULP_fnc_hint;
