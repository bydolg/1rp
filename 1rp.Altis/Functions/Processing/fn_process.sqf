...
if ((count _reqItems) > 0) exitWith {
	[format["Вам нужны следующие предметы для обработки здесь: <t color='#B92DE0'>%1</t>!", _reqItems]] call ULP_fnc_hint;
	false
};
...
if ((count _reqLicenses) > 0) exitWith {
	[format["Вам нужны следующие лицензии для обработки здесь: <t color='#B92DE0'>%1</t>!", _reqLicenses]] call ULP_fnc_hint;
	false
};
...

{
    if ([_x select 0] call ULP_fnc_hasItem < _x select 1) exitWith {
		[format["У вас нет необходимых материалов для обработки здесь: <t color='#B92DE0'>%1</t>!", _materials]] call ULP_fnc_hint;
...
[format["%1 предмет(ов)", getText(_cfg >> "processTitle")], _time, [getPos player, _items, _materials, _cfg, _profession, _leveling], { (player distance (_this select 0)) <= 5 }, {
	...
	["Вы успешно обработали ваши материалы!"] call ULP_fnc_hint;
