/*  Select a random position from an area defined by a marker.
    ...
    Out: position
*/
private ["_area","_water2","_blist","_pos","_empty"];
_area = _this select 0;
_water2 = if (count _this > 1) then {_this select 1} else {0};
...
for [{_i = 0}, {_i < 1000 && !_exit}, {_i = _i + 1}] do {
	...
	if (_shape in ["SQUARE","RECTANGLE"]) then {
	  _pos = _area call SHK_pos_fnc_getPosFromRectangleADS;
	} else {
	  _pos = _area call SHK_pos_fnc_getPosFromEllipseADS;
	};
  ...
  if (count _p > 0) then {
	...
	// Water position
	if (typeName _water2 == "SCALAR") then {
	  switch _water2 do {
	...
	if (count _blist > 0 && _exit) then {
	  // Check each blacklist marker
	  {
		...
		if ([_pos,_x] call SHK_pos_fnc_isBlacklistedADS) exitwith {
		  _exit = false;
		};
	  } foreach _blist;
	};
