...
private ["_shape"];
_shape = _area call SHK_pos_fnc_getMarkerShapeADS;

if (_shape == "ICON") then {
  // Icon has only one position, so if it equals to the given position, then it's blacklisted.
  if ([_pos,_center] call SHK_pos_fnc_isSamePositionADS) then {
    _return = true;
  };
...
// Markers that have an area.
  ...
  if (_shape in ["RECTANGLE","SQUARE"]) then {
    private ["_corners"];
    _corners = _area call SHK_pos_fnc_getMarkerCornersADS;
    ...
    if (_dir % 90 != 0) then {
      ...
      for "_i" from 0 to (count _corners - 1) do {
        ...
        _posNew = [[_shiftedX,_shiftedY],_dir] call SHK_pos_fnc_rotatePositionADS;
    ...
    _return = [_pos,_corners] call SHK_pos_fnc_isInRectangleADS;
  } else {
    if (_shape == "CIRCLE") then {
      _return = [_pos,_area] call SHK_pos_fnc_isInCircleADS;
    } else {
      _return = [_pos,_area] call SHK_pos_fnc_isInEllipseADS;
    };
  };
};
