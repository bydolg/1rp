/*  Select a random position based on anchor position, direction and distance.
    ...
    Out: position
*/
private ["_org","_dst","_dir","_pos","_water2","_road","_empty"];
_org = _this select 0;
_dst = _this select 1;
_dir = if (count _this > 2) then {_this select 2} else {random 360};
_water2 = if (count _this > 3) then {_this select 3} else {0};
...
_pos = [_org,_dst,_dir] call SHK_pos_fnc_getPosADS;
      ...
      if (surfaceIsWater _pos) then {
        ...
        while {_d = _d + 20; _l && _d < 5000} do {
          for "_i" from 0 to 340 step 20 do {
            _p = [_pos,_d,_i] call SHK_pos_fnc_getposADS;
